...
bool CExtractDialog::OnButtonClicked(unsigned buttonID, HWND buttonHWND)
{
  ...
  case IDB_EXTRACT_PASSWORD_BOOK:
  {
    CPasswordBookDialog dialog;
    if (dialog.Create(*this) == IDOK && !dialog.SelectedPassword.IsEmpty())
    {
      _passwordControl.SetText(dialog.SelectedPassword);
    }
    return true;
  }
  case IDB_EXTRACT_PASSWORD_DROPDOWN:
  {
    ShowPasswordDropdownMenu(*this, GetItem(IDB_EXTRACT_PASSWORD_DROPDOWN), _passwordControl);
    return true;
  }
#endif
  }
