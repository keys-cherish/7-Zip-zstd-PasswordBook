...
#include "../FileManager/PasswordBookDialog.h"
...
static LPCSTR const kMethodsNames[] =
    {
        "Copy", "LZMA", "LZMA2", "PPMd", "BZip2", "Deflate", "Deflate64", "PPMd", "FLZMA2", "zstd", "Brotli", "LZ4", "LZ5", "Lizard", "Lizard", "Lizard", "Lizard", "SHA256", "SHA1", "CRC32", "CRC64", "GNU", "POSIX"};

static LPCSTR const kMethodsNamesLong[] =
    {
        "Copy [std]", "LZMA [std]", "LZMA2 [std]", "PPMd [std]", "BZip2 [std]", "Deflate [std]", "Deflate64 [std]", "PPMd [std]", "LZMA2, Fast [std]", "Zstandard", "Brotli", "LZ4", "LZ5", "Lizard, FastLZ4", "Lizard, LIZv1", "Lizard, FastLZ4 + Huffman", "Lizard, LIZv1 + Huffman", "SHA256", "SHA1", "CRC32", "CRC64", "GNU", "POSIX"};
...
static const EMethodID g_7zMethods[] =
    {
        ...
        kLZMA,
        kPPMd,
        kBZip2, kDeflate, kDeflate64, kZSTD, kBROTLI, kLZ4, kLZ5, kLIZARD_M1, kLIZARD_M2, kLIZARD_M3, kLIZARD_M4, kFLZMA2, kCopy};
...
void CCompressDialog::SetMethods(const CObjectVector<CCodecInfoUser> &userCodecs)
{
  ExternalMethods.Clear();
  {
    FOR_VECTOR(i, userCodecs)
    {
      ...
      if (!c.EncoderIsAssigned || !c.IsFilter_Assigned || c.IsFilter || c.NumStreams != 1)
        continue;
      unsigned k;
...
bool CCompressDialog::OnButtonClicked(unsigned buttonID, HWND buttonHWND)
{
  ...
  case IDB_COMPRESS_PASSWORD_BOOK:
  {
    CPasswordBookDialog dialog;
    if (dialog.Create(*this) == IDOK && !dialog.SelectedPassword.IsEmpty())
    {
      _password1Control.SetText(dialog.SelectedPassword);
      _password2Control.SetText(dialog.SelectedPassword);
    }
    return true;
  }
  case IDB_COMPRESS_PASSWORD_DROPDOWN:
  {
    ShowPasswordDropdownMenu(*this, GetItem(IDB_COMPRESS_PASSWORD_DROPDOWN), _password1Control);
    // Also set password2 to match
    UString pwd;
    _password1Control.GetText(pwd);
    _password2Control.SetText(pwd);
    return true;
  }
  }
...
void CCompressDialog::FormatChanged(bool isChanged)
{
  ...
  */
  ...
  EnableItem(IDB_COMPRESS_PASSWORD_BOOK, encrypt);
...
{
  return IsWindowEnabled(GetItem(IDX_COMPRESS_SFX)) && IsButtonCheckedBool(IDX_COMPRESS_SFX);
}
...
bool CCompressDialog::GetFinalPath_Smart(UString &resPath) const
{
  ...
  const bool res = !dirPrefx.IsEmpty() ? NName::GetFullPath(us2fs(dirPrefx), us2fs(name), fullPath) : NName::GetFullPath(us2fs(name), fullPath);
  if (res)
    ...
    case IDC_COMPRESS_DICTIONARY:
      // case IDC_COMPRESS_DICTIONARY2:
      {
        ...
        if ( // blockSizeLog != (UInt32)(Int32)-1 &&
            blockSizeLog != kSolidLog_NoSolid && blockSizeLog != kSolidLog_FullSolid)
        {
...
void CCompressDialog::SetLevel2()
{
  ...
  m_Level.ResetContent();
  {
    ...
    if (index >= 0)
    {
      ...
      if ((fo.Level <= (UInt32)LevelsEnd) || (id != kCopy && fo.Level == Z7_ZSTD_ULTIMATE_LEV) || (id == kZSTD && fo.Level > Z7_ZSTD_FAST_LEV_INC && fo.Level <= Z7_ZSTD_FAST_LEV_INC + 64))
      {
  ...
  for (Int32 i = LevelsStart, ir, j = 0; i <= LevelsEnd; i++)
  {
    ...
    if (ir < 0 && id == kZSTD)
    {
      ...
      {
      case 64:
        lid = IDS_METHOD_ULTIMATEFAST;
        break;
      case 7:
        lid = IDS_METHOD_ULTRAFAST;
        break;
      case 1:
        lid = IDS_METHOD_SUPERFAST;
        break;
      }
      if (lid)
      {
    ...
    else if (ir >= 0 && (LevelsMask & (1 << ir)) && j < Z7_ARRAY_SIZE(g_Levels))
    {
  ...
  { // ultimate level (max possible or zstd --max if allowed)
    UString s;
    if (id == kZSTD)
    {
...
void CCompressDialog::SetMethod2(int keepMethodId)
{
  ...
  for (unsigned m = 0;; m++)
  {
    ...
    if (m < fi.NumMethods)
    {
      ...
      if (is7z)
        if (methodID == kCopy || methodID == kDeflate || methodID == kDeflate64)
          continue;
    }
    else
    {
  ...
  if (!weUseSameMethod)
  {
    ...
    {
      if (defaultLevel >= 10 && defaultLevel <= 19)
        SetNearestSelectComboBox(m_Method, kLIZARD_M1);
      else if (defaultLevel >= 20 && defaultLevel <= 29)
        SetNearestSelectComboBox(m_Method, kLIZARD_M2);
      else if (defaultLevel >= 30 && defaultLevel <= 39)
        SetNearestSelectComboBox(m_Method, kLIZARD_M3);
      else if (defaultLevel >= 40 && defaultLevel <= 49)
        SetNearestSelectComboBox(m_Method, kLIZARD_M4);
    }
    ComprMethodChanged();
    MethodChanged();
  }
...
{
  UString s;
  if (_encryptionMethod.GetCount() > 0 && _encryptionMethod.GetCurSel() != _default_encryptionMethod_Index)
  {
...
static int Combo_AddDict2(NWindows::NControl::CComboBox &cb, size_t sizeReal, size_t sizeShow)
{
  char c = 0;
  unsigned moveBits = 0;
  if ((sizeShow & 0xFFFFF) == 0)
  {
    moveBits = 20;
    c = 'M';
  }
  else if ((sizeShow & 0x3FF) == 0)
  {
    moveBits = 10;
    c = 'K';
  }
...
void CCompressDialog::SetDictionary2()
{
  ...
  {
    {
      _auto_Dict = level <= 4 ? (UInt32)1 << (level * 2 + 16) : level <= sizeof(size_t) / 2 + 4 ? (UInt32)1 << (level + 20)
                                                                                                : (UInt32)1 << (sizeof(size_t) / 2 + 24);
    }

    // we use threshold 3.75 GiB to switch to kLzmaMaxDictSize.
    if (defaultDict != (UInt32)(Int32)-1 && defaultDict >= ((UInt32)15 << 28))
  ...
  case kPPMd:
  {
    ...
    if (defaultDict != (UInt32)(Int32)-1 && defaultDict >= ((UInt32)15 << 28)) // threshold
...
int COptionsDialog::AddPrec(unsigned prec, bool isDefault)
{
  ...
  if (prec == kTimePrec_Win)
    AddTimeOption(s, 100, NsString, "Windows");
  else if (prec == kTimePrec_Unix)
    AddTimeOption(s, 1, SecString, "Unix");
  else if (prec == kTimePrec_DOS)
    AddTimeOption(s, 2, SecString, "DOS");
  else if (prec == kTimePrec_1ns)
    AddTimeOption(s, 1, NsString, "Linux");
  else if (prec == k_PropVar_TimePrec_Base)
    AddTimeOption(s, 1, SecString);
  else if (prec >= k_PropVar_TimePrec_Base)
  {
    UInt32 d = 1;
...
void COptionsDialog::SetTimeMAC()
{
  ...
  const bool m_allow = ai.Flags_MTime();
...
// LangSetDlgItemText(*this, IDX_COMPRESS_TIME_DEFAULT, IDX_COMPRESS_TIME_DEFAULT);
...

  {
    ...
    ShowItem_Bool(IDG_COMPRESS_NTFS,
                  cd->SymLinks.Supported || cd->HardLinks.Supported || cd->AltStreams.Supported || cd->NtSecurity.Supported);
  }
...
bool COptionsDialog::OnButtonClicked(unsigned buttonID, HWND buttonHWND)
{
  switch (buttonID)
  {
  case IDX_COMPRESS_PREC_SET:
  {
    On_CheckBoxSet_Prec_Clicked();
    return true;
  }
  case IDX_COMPRESS_MTIME_SET:
  {
    On_CheckBoxSet_Clicked(MTime);
    return true;
  }
  case IDX_COMPRESS_CTIME_SET:
  {
    On_CheckBoxSet_Clicked(CTime);
    return true;
  }
  case IDX_COMPRESS_ATIME_SET:
  {
    On_CheckBoxSet_Clicked(ATime);
    return true;
  }
  case IDX_COMPRESS_ZTIME_SET:
  {
    On_CheckBoxSet_Clicked(ZTime);
    return true;
  }
  }
  return CModalDialog::OnButtonClicked(buttonID, buttonHWND);
